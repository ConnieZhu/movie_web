...
class ExternalVideo(View):
    ...
    def post(self, request):
        ...
        if not all([name,image,video_type,from_to,nationality,info]):
            return redirect('{}?error={}'.format(reverse_path,
                                                 'Missing required fields'))

        result = check_and_get_video_type(VideoType, video_type,
                                          "Wrong video type")
        ...
        result = check_and_get_video_type(FromType, from_to,
                                          "Wrong video source")
        if result.get('code')!=0:
            return redirect('{}?error={}'.format(reverse_path,
                                                 result['msg']))
        ...
        result = check_and_get_video_type(NationalityType, nationality,
                                          "Wrong nationality")
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse_path,
                                                 result['msg']))
...
class VideoSubView(View):
    ...
    def post(self, request, video_id):
        ...
        if not all([url, number]):
            return redirect('{}?error={}'.format(url_format, 'Missing Necessary Fields'))
        ...
        if not videosub_id:
            ...
            try:
                ...
                return redirect('{}?error={}'.format(url_format, 'Edit Failed'))
        return redirect(reverse('video_sub', kwargs={'video_id': video_id}))
...
class VideoStarView(View):
    ...
    def post(self, request):
        ...
        if not all([name, identity, video_id]):
            return redirect('{}?error={}'.format(path_format, 'Missing Necessary Fields'))

        result = check_and_get_video_type(
            IdentityType, identity, 'Wrong Identity')
